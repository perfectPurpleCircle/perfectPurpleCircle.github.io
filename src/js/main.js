...
function start() {
  ...
  let sorterStyle = document.querySelector('.sorter');
  let startStyle = document.querySelector('.startStyle');
  startStyle.style.display = "none";
  happening.style.display = "flex";
  sorterStyle.style.justifyContent = "space-between"

  /** Hiding options and infos */
  ...
  for (let i = 0; i <= characterDataToSort.length; i++) {
...
function result(imageNum = 3) {
  ...
  document.querySelector('.happening').style.display = "none";
  ...
  document.querySelector('.options').style.display = 'none';
  document.querySelector('.info').style.display = 'hidden';
  ...
  const imgRes = (char, num) => {
    ...
    return `<div class="result image"><div class="left"><span>${num}</span></div><div class="right"><img src="${char.img}"><div class="nameStyle"><span title="${charTooltip}">${charName}</span></div></div></div>`;
  }
  const res = (char, num) => {
    const charName = reduceTextWidth(char.name, 'Arial 12px', 160);
    const charTooltip = char.name !== charName ? char.name : '';
